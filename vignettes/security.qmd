...
Unfortunately, Plumber does not implement HTTPS support natively, but most hosting options offer ways to deploy HTTPS and HSTS in front of your Plumber API.
...
The code above will apply the default settings to your API. However, we urge the developer to spend some time getting acquainted with the concepts around the various headers. Especially, some of them are only relevant if your API is serving HTML pages, which many plumber2 APIs aren't, and these can thus by turned off to save a bit on the packet size and thus improve latency. You can read more about all of the headers as well as understand which are relevant when by reading the documentation for `firesafety::SecurityHeaders`.

## Cross-Origin Resource Sharing (CORS)

Cross-Origin Resource Sharing is a technology to control which webpages are allowed to access resources hosted on other domains. In general, you will try to limit this as much as possible to avoid webpages imitating other webpages to spoof the end user, or for other reasons. Since it is generally a good idea, CORS is turned off by default. This means that if you have a webpage that needs to communicate with your API (and that API is hosted on another domain), then you need to allow CORS.

Setting up CORS can be done both with annotation and with code. The annotation 
approach is both easy and allows you to keep CORS settings close to where the endpoint is defined. You set it up by adding a `@cors` tag to your annotation followed by who is allowed to interact with the endpoint. This can be either `*`
to allow everyone, or a specific origin to whitelist.
...
The annotation is somewhat limited and if you need more power to define the CORS specification you can do so programmatically with the `api_security_cors()`
function. With that you can also specify which headers are allowed and which will be exposed, along with allowing credentials to pass through and set the maximum time the CORS agreement can be kept in the cache.

You can read more about CORS in the documentation for `firesafety::CORS`.

## Resource Isolation Policies

Setting up resource isolation policies can complement CORS be denying access to resources that are not protected by CORS. It prevents, at the server level, the access to resources and is thus not a technology that relies on browser implementations (which CORS does). It's important to state that it is not a decision between CORS or resource isolation but two sides of the same coin.

Like CORS, resource isolation can be set up with annotation using the `@rip`. Rather than providing an allowed origin like for CORS, you specify the allowance level (`cross-site`, `same-site`, or `same-origin`). Also, like with
CORS, you have even more control if using it programmatically with `api_security_resource_isolation()`.

You can read more about resource isolation policies at the documentation for `firesafety::ResourceIsolation`.
