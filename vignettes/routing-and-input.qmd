...
This would give your handler access to all parsers registered with plumber2 but will select `artisanal_json_parser` over `json` if the content type is `application/json`. You register new parsers using `register_parser()`, but can also provide them directly in the block annotation like so:
